...
- SR-IOV 개념 (PF / VF 구조)
- IOMMU / VFIO 개념
- Passthrough vs MIG > Passthrough : VM에 GPU 전체를 직접 할당 (VFIO / IOMMU 사용)
- Passthrough vs MIG > MIG : GPU를 논리적으로 나눠서 여러 VM에 할당 
- MIG 개념 및 GI/CI 구조
- mdev 동작 방식
...
- dcgm-exporter 동작 방식
- DCGM이 뭔지 (Data Center GPU Manager)
- host 레벨과 instance 레벨 모니터링 차이 
- qemu-guest-agent 동작 방식 및 호스트-게스트 통신 구조

- MIG > MIG(Multi-Instance GPU)가 뭔지, 왜 쓰는지
- MIG > GI(GPU Instance)와 CI(Compute Instance)
- MIG > MIG profile 종류
- mdev > mdev(Mediated Device)가 뭔지 
- mdev > GPU Passthrough와 MIG mdev 방식의 차이
- mdev > orphan mdev가 왜 생기는지
